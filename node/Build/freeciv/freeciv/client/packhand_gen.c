...
bool client_handle_packet(enum packet_type type, const void *packet)
{
  ...
  case PACKET_DIPLOMACY_CREATE_CLAUSE:
    handle_diplomacy_create_clause(
      ...
      ((const struct packet_diplomacy_create_clause *)packet)->clause_type,
  ...
  case PACKET_DIPLOMACY_REMOVE_CLAUSE:
    handle_diplomacy_remove_clause(
      ...
      ((const struct packet_diplomacy_remove_clause *)packet)->clause_type,
  ...
  case PACKET_INFO_TEXT_MESSAGE:
    handle_info_text_message(
      ((const struct packet_info_text_message *)packet)->message);
    return TRUE;

  case PACKET_GOTO_PATH:
    handle_goto_path(
      ((const struct packet_goto_path *)packet)->unit_id,
      ((const struct packet_goto_path *)packet)->length,
      ((const struct packet_goto_path *)packet)->dir,
      ((const struct packet_goto_path *)packet)->dest,
      ((const struct packet_goto_path *)packet)->turns);
    return TRUE;
